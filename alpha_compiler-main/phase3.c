...
expr* lvalue_expr(SymbolTableEntry* sym){
    ...
    e->next = (expr*) 0;
    e->sym = sym;
    printf("lvalue_expr: sym->name = %s, sym->type = %d\n", sym->s_name, sym->type);
    ...
    switch(sym->type){
        ...
        case programfunc_s:
            ...
            break;
        case libraryfunc_s:
	    printf("lvalue_expr: setting type to libraryfunc_e\n");
...
expr* make_call(expr* lv, expr* reversed_elist, int scope, int yylineno){
    ...
    if (!func) {
        fprintf(stderr, "make_call: func is NULL\n");
    } else {
        fprintf(stderr, "make_call: func->type = %d\n", func->type);
        if (!func->sym) {
            fprintf(stderr, "make_call: func->sym is NULL\n");
        } else {
            fprintf(stderr, "make_call: func->sym->name = %s\n", func->sym->s_name);
        }
    }
